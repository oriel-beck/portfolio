...
export default function Header({ activeSection }: { activeSection?: string | null }) {
    return (
        <header className="flex w-full flex-row fixed top-0 right-0" style={{ height: '70px', backgroundColor: 'rgba(40, 40, 40, 0.70)' }}>
            <nav role='navigation' className='flex flex-row items-center ml-5 md:ml-12'>
                <ul className='flex flex-row justify-center space-x-5 md:space-x-7 text-white'>
                    <li><a className={activeSection === "about" ? 'link active' : 'link'} href="#about">About</a></li>
                    <li><a className={activeSection === "projects" ? 'link active' : 'link'} href="#projects">Projects</a></li>
                    <li><a className={activeSection === "contact" ? 'link active' : 'link'} href="#contact">Contact</a></li>
                </ul>
            </nav>
